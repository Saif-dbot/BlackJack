#!/usr/bin/env python3
"""Enhanced training script for card-counting agents with visualization and reporting."""
...
from src.agents.counting import (
    MonteCarloCountAgent, QLearningCountAgent, SARSACountAgent, DQNCountAgent
)
...
def evaluate_agent(env: BlackjackEnv, agent, num_episodes: int = 1000) -> dict:
    ...
    Args:
        env: Blackjack environment with counting
        agent: Trained agent
...
def train_agent(
    ...
    """Train counting agent using configuration file.
    ...
    models_dir = output_dir / "models" / "counting"
    ...
    logger = setup_logger(f"train_{agent_type}_counting", logs_dir)
    logger.info(f"Starting enhanced training for {agent_type} with card counting")
    logger.info(f"Configuration: {config}")
    
    # Create environment with counting enabled
    ...
    env = BlackjackEnv(deck_config=deck_config, enable_counting=True)
    ...
    agent_classes = {
        ...
        "dqn_count": DQNCountAgent,
    }
    
    # Handle both naming conventions - ensure "_count" suffix
    agent_type_normalized = agent_type
    if not agent_type_normalized.endswith("_count"):
        agent_type_normalized = f"{agent_type}_count"
    
    if agent_type_normalized not in agent_classes:
        raise ValueError(f"Unknown agent type: {agent_type_normalized}. Available: {list(agent_classes.keys())}")
    
    agent = agent_classes[agent_type_normalized](config["hyperparameters"])
    ...
    print(f"{'='*50}")
    print(f"Agent Type: {agent_type} (with Card Counting)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train card-counting Blackjack agents with enhanced features"
    )
    parser.add_argument(
        "--config",
        type=str,
